...
(deftest simple-conversions
  (let [int-conv (conversion-for-type java.lang.Integer (partial + 1))
    ...
    (testing "seq-conversion"
      (let [convert (make-converter int-conv double-conv seq-conversion)]
        (is (= (+ 1 3) (convert 3)))
        (is (= [(+ 1 3) (+ 2.0 5.0)] (convert (map identity [3 5.0]))))))))

(deftest mixed-conversions
  (let [int-conv (conversion-for-type java.lang.Integer (partial + 1))
        double-conv (conversion-for-type java.lang.Double (partial + 2.0))
        conversions (concat [int-conv double-conv] sequence-conversions)
    ...
    (is (= [four [seven] [[(list four)]] [{[] (list seven)}]]
           (convert (map identity [3 [5.0] [[(list 3)]] [{[] (list 5.0)}]]))))))
